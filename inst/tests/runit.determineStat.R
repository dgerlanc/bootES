test.determineStat <- function() {
  ...
  test.result = bootES:::determineStat(test, effect.type="unstandardized")
  checkEquals(test.result, 'mean')

  ## Assert that Three Groups, One Measure, Unstandardized yields mean
  ## Assert: stat is mean
  test.inut = data.frame(score=c(g1, g2, g3), group=threeGrpLabels)
  test.result = bootES:::determineStat(test.inut$score, test.inut$group,
